...
DISK_ATTACH_STATUS = frozenset(["ATTACHED", "ATTACHING", "DETACHED", "DETACHING"])
...
class OciDisk(disk.BaseDisk):
    ...
    def ExecuteAttachIscsiCommands(self, vm):
        vm.RemoteCommand(
            f"sudo iscsiadm -m node -o new -T {self.iqn} -p {self.ipv4}:{self.port}"
        )
        vm.RemoteCommand(
            f"sudo iscsiadm -m node -o update -T {self.iqn} -n node.startup -v automatic"
        )
        vm.RemoteCommand(
            f"sudo iscsiadm -m node -T {self.iqn} -p {self.ipv4}:{self.port} -l"
        )

    def ExecuteDetachIscsiCommands(self, vm):
        vm.RemoteCommand(
            f"sudo iscsiadm -m node -T {self.iqn} -p {self.ipv4}:{self.port} -u"
        )
        vm.RemoteCommand(
            f"sudo iscsiadm -m node -o delete -T {self.iqn} -p {self.ipv4}:{self.port}"
        )
